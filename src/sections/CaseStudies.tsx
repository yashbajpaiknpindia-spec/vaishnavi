...
const caseStudies = [
  ...
  {
    company: "Reliance Industries", logo: "RELIANCE",
  ...
  {
    company: "Adani Group", logo: "ADANI",
...
function CaseStudyRow({ study, index }: { study: typeof caseStudies[0]; index: number }) {
  ...
  const imgX = useTransform(scrollYProgress, [0, 1], [index % 2 === 0 ? '3%' : '-3%', index % 2 === 0 ? '-3%' : '3%']);

  return (
    <motion.div
      ref={ref}
      style={{ scale }}
      className="border-b border-ink/10 group overflow-hidden"
    >
      <div className="grid-container py-12 md:py-24 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div className={`lg:col-span-5 space-y-8 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 bg-ink text-paper flex items-center justify-center font-black text-[9px] tracking-tighter">
                {study.logo}
              </div>
              <h4 className="text-xs font-mono uppercase tracking-widest opacity-40">{study.company}</h4>
            </div>
            <h3 className="text-4xl md:text-6xl tracking-tight mb-8">{study.title}</h3>
        ...
        <div className={`lg:col-span-7 ${index % 2 === 1 ? 'lg:order-1' : ''}`}>
          <div className="aspect-[16/9] overflow-hidden relative">
            ...
            >
              <div className="absolute inset-0 opacity-[0.06] flex items-center justify-center overflow-hidden pointer-events-none z-10">
                <span className="text-[15vw] font-black uppercase tracking-tighter">{study.logo}</span>
              </div>
              <img
                src={`https://picsum.photos/seed/${study.company.replace(' ', '')}/1200/800`}
                alt={study.company}
    ...
    <section className="border-b border-ink/10">
      <div className="grid-container py-24">
        ...
        <motion.h2
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
          className="text-4xl md:text-6xl font-black mt-8"
        >
          Proven Impact for <br />Industry Leaders.
        </motion.h2>
      </div>

      <div className="border-t border-ink/10">
        {caseStudies.map((study, index) => (
          <CaseStudyRow key={index} study={study} index={index} />
        ))}
      </div>
    </section>
  );
